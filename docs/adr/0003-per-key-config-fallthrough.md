...
As part of making every level accept identical keys, the global `default_layout` key was renamed to `layout`. The old key triggers a hard load-time error instead of a silent ignore (which would quietly reset users to the built-in layout) or a permanent alias.
